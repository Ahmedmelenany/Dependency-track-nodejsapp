...
After the SBOM is generated by cdxgen, the pipeline validates it against the official CycloneDX JSON schema using [cyclonedx-cli](https://github.com/CycloneDX/cyclonedx-cli). This catches malformed or corrupted SBOMs before they're uploaded to Dependency-Track.
...
| Flag | Purpose |
|---|---|
| `--input-version v1_4` | Must match the `--spec-version` used by cdxgen — keep these in sync |
| `--fail-on-errors` | Stops the pipeline immediately on schema violations |

Without this step, a broken SBOM would still be uploaded and silently rejected (or partially processed) by Dependency-Track, leaving you with a stale risk score and a falsely-passing pipeline.

### Dependency-Track risk gate

The pipeline doesn't just upload the SBOM and move on - it waits for Dependency-Track to finish analysing the components, then enforces a configurable risk threshold. If the project's inherited risk score exceeds the threshold, the pipeline fails and the image is **not** pushed to ECR.

The gate runs in six steps:

```
1. POST /api/v1/bom                                  → upload SBOM, capture token
2. GET  /api/v1/bom/token/{token}                    → poll until processing=false
3. GET  /api/v1/project/lookup                       → resolve project UUID
4. POST /api/v1/metrics/project/{uuid}/refresh       → force metrics refresh
5. GET  /api/v1/metrics/project/{uuid}/current       → read inherited risk score
6. Compare to RISK_SCORE_THRESHOLD                   → fail pipeline if exceeded
```

| Variable | Default | Purpose |
|---|---|---|
| `RISK_SCORE_THRESHOLD` | `10` | Maximum acceptable inherited risk score - configurable in pipeline variables |
| `TIMEOUT_SECONDS` | `300` | Max wait time for SBOM processing (polled every 5s) |

The metrics refresh step is important: Dependency-Track caches metrics and only updates them on a scheduled task. Without forcing a refresh, the gate would read stale numbers from before the SBOM upload.

Before failing, the step prints a full risk report so the developer can see exactly what triggered the failure:

```
===== Dependency-Track Risk Report =====
Project:              node-app @ 1.0.0
Inherited Risk Score: 42 (threshold: 10)
Vulnerabilities:      3 critical | 5 high | 2 medium | 1 low
========================================
```

> **Note:** the intentional CVEs in this demo will produce a risk score well above 10, so the gate will fail by default - that's the point. To see a passing build, raise `RISK_SCORE_THRESHOLD` or suppress findings in the Dependency-Track UI.
